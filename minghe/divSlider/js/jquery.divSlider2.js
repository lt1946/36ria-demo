	...
	yijs.DivSlider.prototype = {
		...
		showFull : function(index){
			...
			if(this.current == null){
				...
				if(index == this.$items.size() -1) _showWidth = $current.parent().width() - parseInt($current.css("left"));
				var _hideWidth = _w - _showWidth > 0 && _w - _showWidth || 0;
				this.currentHideWidth = _hideWidth;
				var _d = _hideWidth;
				var _itemMoveDistance = Math.floor(_d/$current.siblings().size());
				if($current.prevAll().size() > 0){
					var _currentDistance = _itemMoveDistance * ($current.prevAll().size());
					_d -= _currentDistance;
					_that.animateTo($current,0,parseInt($current.css("left")) - _currentDistance);
					$current.prevAll().each(function(){
						if(0 != _that.$items.index($(this))){
							var _l = parseInt($(this).css("left"));
							_l -= _itemMoveDistance;
							_that.animateTo($(this),0,_l);						
						}
				...
				if($current.nextAll().size() > 0){
					$current.nextAll().each(function(){
						if(_that.$items.size()-1 != _that.$items.index($(this))){
							var _l = parseInt($(this).css("left"));
							_l += _d;
							_that.animateTo($(this),0,_l);
							_d -= _itemMoveDistance;						
						}
					})
				}			
			}else{
